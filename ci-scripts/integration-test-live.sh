...
# Runs "live"-mode tests against a samos node that is already running
...
# The samos node must be run with -enable-wallet-api=true
...
echo "checking if samos node is running"

http_proxy="" https_proxy="" wget -O- $HOST 2>&1 >/dev/null

if [ ! $? -eq 0 ]; then
    echo "Samos node is not running on $HOST"
    exit 1
fi
...
SAMOS_INTEGRATION_TESTS=1 SAMOS_INTEGRATION_TEST_MODE=$MODE RPC_ADDR=$RPC_ADDR SAMOS_NODE_HOST=$HOST \
