...
```bash
$ PROG=samos-cli source $GOPATH/src/github.com/samoslab/samos/cmd/cli/autocomplete/bash_autocomplete
```
...
The default CLI wallet dir is located in `$HOME/.samos/wallets/`, change it by setting the
...
The default CLI wallet file name is `samos_cli.wlt`, change it by setting the `WALLET_NAME` env.
...
After the installation, you can run `samos-cli` to see the usage:

```
$ samos-cli

NAME:
   samos-cli - the samos command line interface

USAGE:
   samos-cli [global options] command [command options] [arguments...]
...
COMMANDS:
     ...
     addressGen            Generate samos or bitcoin addresses
     ...
     send                  Send samos from a wallet or an address to a recipient address
     status                Check the status of current samos node
     transaction           Show detail info of specific transaction
     verifyAddress         Verify a samos address
     version
     walletBalance         Check the balance of a wallet
     walletDir             Displays wallet folder address
     walletHistory         Display the transaction history of specific wallet. Requires samos node rpc.
...
### Add Private Key
Add a private key to a samos wallet.

```bash
$ samos-cli addPrivateKey [command options] [private key]
```

```
OPTIONS:
    -f value [wallet file or path] private key will be added to this wallet
    if not specified then default wallet ($HOME/.samos/wallets//wallets/samos_cli.wlt)
    will be used
```

#### Example
```bash
$ samos-cli addPrivateKey -f $WALLET_PATH $PRIVATE_KEY
```
...
```bash
$ samos-cli addressBalance [addresses]
```

#### Example
```bash
$ samos-cli addressBalance 2iVtHS5ye99Km5PonsB42No3pQRGEURmxyc 2GgFvqoyk9RjwVzj8tqfcXVXB4orBwoc9qv
```
<details>
...
Generate new samos or bitcoin addresses.

```bash
$ samos-cli addressGen [command options] [arguments...]
```
...
OPTIONS:
        ...
        --bitcoin, -b            Output the addresses as bitcoin addresses instead of samos addresses
...
#### Examples
##### Generate `n` number of samos addresses
```bash
$ samos-cli addressGen --count 2
```
...
```bash
$ samos-cli addressGen --count 2 --bitcoin
```
...
```bash
$ samos-cli addressGen --count 2 --hide-secret
```
...
```bash
$ samos-cli addressGen --count 2 --only-addr
```
...
```bash
$ samos-cli addressGen --count 2 --seed "my super secret seed"
```
...
```bash
samos-cli addressGen --count 2 --hex
```
...
```bash
$ samos-cli addressOutputs [address list]
```

#### Example
```bash
samos-cli addressOutputs tWPDM36ex9zLjJw1aPMfYTVPbYgkL2Xp9V 29fDBQuJs2MDLymJsjyWH6rDjsyv995SrGU
```
...
```bash
$ samos-cli blocks [starting block or single block seq] [ending block seq]
```

#### Example
```bash
$ samos-cli blocks 41 42
```
...
Checks if the given database file contains valid samos blockchain data
If no argument is given, the default `data.db` in `$HOME/.$COIN/` will be checked.

```bash
$ samos-cli checkdb [db path]
```

#### Example
```bash
$ samos-cli checkdb $DB_PATH
```
...
```bash
$ samos-cli createRawTransaction [command options] [to address] [amount]
```
...
```bash
$ samos-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending to a specific change address

```bash
$ samos-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ samos-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
```bash
$ samos-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ samos-cli decodeRawTransaction [raw transaction]
```

Decode a raw samos transaction.

#### Example

```bash
samos-cli decodeRawTransaction 
```
...
Broadcast a raw samos transaction.
Output is the transaction id.

```bash
$ samos-cli broadcastTransaction [raw transaction]
```

```bash
$ samos-cli broadcastTransaction 
```
<details>
 <summary>View Output</summary>
...
### Generate a wallet
Generate a new samos wallet.

```bash
$ samos-cli generateWallet [command options]
```
...
OPTIONS:
        ...
        -f value  [walletName] Name of wallet. The final format will be "yourName.wlt".
                             If no wallet name is specified a generic name will be selected. (default: "samos_cli.wlt")
...
```bash
$ samos-cli generateWallet -s "this is the super secret seed everyone needs but does not have"
```
...
```bash
$ samos-cli generateWallet -f "secret_wallet.wlt"
```
...
```bash
$ samos-cli generateWallet -l "cli wallet"
```
...
Generate new addresses for a samos wallet.

```bash
$ samos-cli generateAddresses [command options]
```

```
OPTIONS:
        -n value    [numberOfAddresses]  Number of addresses to generate (default: 1)
        -f value    [wallet file or path] Generate addresses in the wallet (default: $HOME/.samos/wallets//wallets/samos_cli.wlt)
        --json, -j  Returns the results in JSON format
```

##### Generate an address for the default wallet
```bash
$ samos-cli generateAddresses
```
...
```bash
$ samos-cli generateAddresses $WALLET_PATH
```
...
```bash
$ samos-cli generateAddresses -n 2
```
...
##### Generate a JSON output
```bash
$ samos-cli generateAddresses --json
```
...
### Last blocks
Show the last `n` samos blocks.
By default the last block is shown.

```bash
$  samos-cli lastBlocks [numberOfBlocks]
```

#### Examples
##### Get the last block
```bash
$ samos-cli lastBlocks
```
...
List addresses in a samos wallet.

```bash
$ samos-cli listAddresses [walletName]
```

If no `walletName` is given then default wallet ($HOME/.samos/wallets/samos_cli.wlt) is used.

> NOTE: The wallet name `samos_cli.wlt` or full path `$HOME/.samos/wallets/samos_cli.wlt` can be used.
        When only the wallet name is given then the default wallet dir, $HOME/.$COIN/wallets is used.

#### Examples
##### List addresses of default wallet
```bash
$ samos-cli listAddresses
```
...
```bash
$ samos-cli listAddresses $WALLET_NAME or $WALLET_PATH
```
...
### List wallets
List wallets in the samos wallet directory.

```bash
$ samos-cli listWallets
```
...
### Send
Make a samos transaction.

```bash
$ samos-cli send [command options] [to address] [amount]
```

```
OPTIONS:
        -f value    [wallet file or path] From wallet. If no path is specified your default wallet
                    (`$HOME/.samos/wallets/samos_cli.wlt`) path will be used.
        -a value    [address] From address
...
```bash
$ samos-cli send $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending from a specific wallet
```bash
$ samos-cli send -f $WALLET_PATH $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending from a specific address in a wallet
```bash
$ samos-cli send -f $WALLET_PATH -a $FROM_ADDRRESS $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ samos-cli send -f $WALLET_PATH -a $FROM_ADDRESS -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending to multiple addresses
```bash
$ samos-cli send -f $WALLET_PATH -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
##### Generate a JSON output
```bash
$ samos-cli send -f $WALLET_PATH -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ samos-cli transaction [transaction id]
```

#### Example
```bash
$ samos-cli transaction 824d421a25f81aa7565d042a54b3e1e8fdc58bed4eefe8f8a90748da6d77d135
```
...
### Verify address
Verify whether a given address is a valid samos addres or not.

```bash
$  samos-cli verifyAddress [samos address]
```
...
```bash
$ samos-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda
```
...
###### Invalid checksum
```bash
$ samos-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsdx
```
...
Check the wallet a samos wallet.

```bash
$ samos-cli walletBalance [wallet]
```

> NOTE: Both the full wallet path or only the wallet name can be used.
        If no wallet is specified then the default wallet: `$HOME/.$COIN/wallets/samos_cli.wlt` is used.

#### Example
##### Balance of default wallet
```bash
$ samos-cli walletBalance
```
...
```bash
$ samos-cli walletBalance 2018_04_01_198c.wlt
```
*OR*

```bash
$ samos-cli walletBalance ~/.samos/wallets/2018_04_01_198c.wlt
```
...
Get the current samos wallet directory.

```bash
$ samos-cli walletDir [command options]
```
...
```bash
$ samos-cli walletHistory [command options]
```
...
##### Default wallet
```bash
$ samos-cli walletHistory
```

##### Specific wallet
```bash
$ samos-cli walletHistory -f $WALLET_NAME
```
*OR*
```bash
$ samos-cli walletHistory -f $WALLET_PATH
```
...
```bash
$ samos-cli walletOutputs [wallet file]
```

#### Examples
##### Default wallet
```bash
$ samos-cli walletOutputs
```

##### Specific wallet
```bash
$ samos-cli walletHistory $WALLET_NAME
```
*OR*
```bash
$ samos-cli walletHistory $WALLET_PATH
```
...
### CLI version
Get version of current samos cli.

```bash
$ samos-cli version [command options]
```
...
```bash
$ samos-cli send $RECIPIENT_ADDRESS $AMOUNT -c $CHANGE_ADDRESS
```
...
```bash
$ samos-cli send -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```
